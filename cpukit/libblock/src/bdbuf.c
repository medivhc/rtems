...
typedef struct rtems_bdbuf_cache
{
  ...
  rtems_chain_control free_list;         /**< Buffers in FREE state */
...
/**
 * The replacement policy function.
 */
static rtems_status_code (*rtems_bdbuf_init_policy)(const rtems_bdbuf_config *config,
  rtems_chain_control *free_list);
static rtems_bdbuf_buffer *(*rtems_bdbuf_victim_next)(rtems_bdbuf_buffer *);
static void (*rtems_bdbuf_enqueue)(rtems_bdbuf_buffer *);
static void (*rtems_bdbuf_dequeue)(rtems_bdbuf_buffer *);
...
rtems_bdbuf_show_usage (void)
{
  ...
  val = rtems_bdbuf_list_count (&bdbuf_cache.free_list);
  printf (", free=%lu", val);
  total = val;
...
static rtems_bdbuf_buffer *
rtems_bdbuf_get_buffer_from_free_list_and_queue (rtems_disk_device *dd,
                                      rtems_blkdev_bnum  block)
{
...
rtems_bdbuf_init (void)
{
  ...
  rtems_chain_initialize_empty (&bdbuf_cache.free_list);
  ...
  /*
   * Set the bdbuf replacement policy entry points.
   */
                                       ...
                                       rtems_blkdev_bnum  block)
{
  ...
  if (bd == NULL)
  {
    bd = rtems_bdbuf_get_buffer_from_free_list_and_queue (dd, block);
