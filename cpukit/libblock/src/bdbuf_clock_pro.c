...
/**
* @brief non_resident_buffer keeps the buffer dd and block information.
* It uses rbtree to help to seacrh the buffer.
*/
typedef struct
{
  rtems_rbtree_node rbtree_node;  /**< rbtree_node that helps to search  */
  uintptr_t dd;                   /**< block device*/
  rtems_blkdev_bnum block;        /**< block number*/
  struct clock_pro_block *ptr;    /**< pointer to clock_pro_block */
} non_resident_buffer;

/**
* @brief clock_pro_block contains either rtems_bdbuf_buffer or 
* non_resident_buffer.
*/
typedef struct clock_pro_block
{
  ...
  rtems_chain_node link; /**<  the node for circular list   */
  uint8_t flags;  /**< flags for clock_pro_block  */
} clock_pro_block;
...
typedef struct
{
  int cached_buffer_num;   /**< number of buffer in CACHED state */
  int cached_buffer_min;   /**< the min number of the buffer in CACHED state */
  int non_resident_num;    /**< number of non_resident_buffer */
  int non_resident_max;    /**< the max number of non_resident_buffer.*/
  int hot_num;             /**< number of hot buffer */
  int hot_max;             /**< max number of hot buffer */
  rtems_rbtree_control non_resident_rbtree;   /**< rbtree control*/
  rtems_chain_control free_non_resident_list; /**< non_resident_buffer which 
  is not in the list */
  rtems_chain_control free_resident_list;  /**< clock_pro_block which is not 
  in the list */
  rtems_chain_node *hand_hot;  /**< hand hot */
  rtems_chain_node *hand_cold; /**< hand cold */
  rtems_chain_node *hand_test; /**< hand test */
  clock_pro_block *init_block; /**< the first block in the circular list */
  clock_pro_block *blocks;     /**< pointer to the blocks*/
  non_resident_buffer *non_resident; /** <pointer to the non_resident_buffer*/
} clock_pro_state;
...
/*
 * compare function for non_resident 
 */
static int
...
static void
check_conflict_with_hands (rtems_chain_node * node)
{
                 ...
                 rtems_chain_control * free_list)
{
  ...
  /*
   * init_block is neither cached buffer nor non_resident buffer
   */
  ...
  clock_pro.cached_buffer_num = 0;
  clock_pro.hot_num = 0;
  clock_pro.hot_max = clock_pro.non_resident_max / 8;
  clock_pro.cached_buffer_min = 2 * clock_pro.hot_max;
...
_clock_pro_victim_next (rtems_bdbuf_buffer * pre)
{
  ...
  /*
   * More cached buffer means less modified buffer, but we have less control
   * of modified buffer. If there is no cached buffer,
   * the algorithm is meaningless. So the cached_buffer_min is set.
   */
  if (clock_pro.cached_buffer_num <= clock_pro.cached_buffer_min) {
    return NULL;
  }
  ...
  /*
   * check cold buffer without ref bit
   */
  chain_node = rtems_chain_next (chain_node);
  while (rtems_chain_next (chain_node) != victim_start) {
    cb = container_of (chain_node, clock_pro_block, link);
...
rotate_hand_cold_over (clock_pro_block * cb)
{
  ...
  while (!rtems_chain_are_nodes_equal (chain_node, &cb->link)) {
    if (is_cached_buffer (cb)
      ...
      if (is_ref_buffer (cb)) {
        /*
         * this buffer should be upgraded to hot, we get the previous node first.
         */
        chain_node = rtems_chain_previous(chain_node);
        move_after_hand_hot (current, true);
      } else {
        /*
         * This happens that the buffer is not swapped out because it is waiting, 
...
find_non_resident_buffer (rtems_bdbuf_buffer * bd)
{
  ...
  rtems_rbtree_node *node =
    rtems_rbtree_find_unprotected (&clock_pro.non_resident_rbtree,
        &target.rbtree_node);
  if (node == NULL) {
    return NULL;
  } else {
...
change_to_non_resident (rtems_bdbuf_buffer * bd)
{
  ...
  clock_pro_block *cb = bd->policy_user;
  rtems_chain_node *chain_node;
  ...
  /* insert the non_resident after the bd, the bd will be removed then */
  ...
  switch (bd->state) {
    case RTEMS_BDBUF_STATE_EMPTY:
      if (is_cached_buffer (cb)) {
        ...
        /*
         * the buffer which is going to be swapped out will be add to
         * non_resident
         */
